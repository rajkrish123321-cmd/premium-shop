...
export default function TrendyJewelleryStore() {
	...
	const change = (id: string, delta: number) => setCart(old => {
		...
		const result = { ...old };
		if (next) { result[id] = next; if (!old[id]) setSelectedItems(prev => ({ ...prev, [id]: true })); }
	...
	const handlePayment = async () => {
		if (!isFormValid || !(await loadRazorpay())) return alert("Razorpay failed to load.");
	...
	const field = (placeholder: string, value: string, setter: (value: string) => void, type = "text") => <input className="field" type={type} placeholder={placeholder} value={value} onChange={e => setter(e.target.value)} />;
	return <main className="store"><style dangerouslySetInnerHTML={{ __html: `*{box-sizing:border-box;font-family:Arial;margin:0}body{background:#fdfbf7;color:#111}.store{min-height:100vh}.header{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;align-items:center;padding:20px 5%;background:#fff;border-bottom:1px solid #eee}.brand{color:#b38728;letter-spacing:2px}.btn,.field{padding:12px;border-radius:6px;border:1px solid #ddd}.btn{cursor:pointer;font-weight:bold}.dark{background:#111;color:#fff}.content{max-width:1250px;margin:auto;padding:50px 20px}.products{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:25px}.card,.panel{background:#fff;border:1px solid #eee;border-radius:12px;padding:15px}.card img{width:100%;height:320px;object-fit:cover;border-radius:8px}.price{color:#b38728;font-size:22px;font-weight:bold}.checkout-container{max-width:900px;margin:auto;display:grid;gap:30px}@media(min-width:768px){.checkout-container{grid-template-columns:1.2fr 1fr}}.field{width:100%;margin-bottom:15px}.cart-item{display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid #eee}.modal-overlay{position:fixed;inset:0;background:#0008;display:flex;align-items:center;justify-content:center;z-index:5}.modal-content{background:#fff;padding:35px;border-radius:14px;width:90%;max-width:400px}` }} /><div style={{ background: "#111", color: "#d4af37", padding: 10, textAlign: "center" }}>✨ FREE SHIPPING ON ORDERS ABOVE ₹799 ✨</div><header className="header"><h1 className="brand" onClick={() => setCheckout(false)}>TRENDY JEWELLERY</h1><div>{userName ? `Hi, ${userName}` : <button className="btn" onClick={() => setShowLogin(true)}>LOGIN</button>} <button className="btn dark" onClick={() => count && setCheckout(true)}>🛒 CART ({count})</button></div></header>{showLogin && <div className="modal-overlay"><div className="modal-content"><button onClick={() => setShowLogin(false)}>×</button><h2>Welcome</h2>{field("Full Name", name, setName)}{field("Mobile Number", phone, v => setPhone(v.replace(/\D/g, "").slice(0, 10)))}<button className="btn dark" onClick={handleLogin}>Verify & Login</button></div></div>}<div className="content">{!checkout ? <><section style={{ textAlign: "center", marginBottom: 40 }}><h2>Elegance, <span style={{ color: "#b38728" }}>Redefined.</span></h2><p>Handcrafted premium jewellery for your special moments.</p></section><section className="products">{products.map(p => <article className="card" key={p.id}><img src={p.image} alt={p.name} /><h3>{p.name}</h3><p className="price">₹{p.price.toLocaleString("en-IN")}</p>{cart[p.id] ? <div><button onClick={() => change(p.id, -1)}>−</button> {cart[p.id]} <button onClick={() => change(p.id, 1)}>+</button></div> : <button className="btn" onClick={() => change(p.id, 1)}>Add to Cart</button>} <button className="btn dark" onClick={() => { change(p.id, 1); setCheckout(true); }}>Buy Now</button></article>)}</section></> : <><button className="btn" onClick={() => setCheckout(false)}>← Back to Shopping</button><div className="checkout-container"><div className="panel"><h2>Your Cart</h2>{products.filter(p => cart[p.id]).map(p => <div className="cart-item" key={p.id}><input type="checkbox" checked={!!selectedItems[p.id]} onChange={() => toggleSelection(p.id)} /> <img src={p.image} alt={p.name} width="60" /> <span>{p.name} ₹{p.price}</span><button onClick={() => change(p.id, -1)}>−</button>{cart[p.id]}<button onClick={() => change(p.id, 1)}>+</button></div>)}</div><div className="panel"><h2>🔒 Secure Checkout</h2>{field("Full Name", name, setName)}{field("Email Address", email, setEmail, "email")}{field("Mobile Number", phone, v => setPhone(v.replace(/\D/g, "").slice(0, 10)))}{field("Complete Shipping Address", address, setAddress)}{field("Pincode", pincode, v => setPincode(v.replace(/\D/g, "").slice(0, 6)))}<h2>Total: ₹{total.toLocaleString("en-IN")}</h2><button className="btn dark" disabled={!isFormValid} onClick={handlePayment}>PAY SECURELY</button></div></div></>}</div></main>;
}
